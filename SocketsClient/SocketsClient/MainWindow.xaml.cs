...
namespace SocketsClient
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private static string ServerCertificateName = "X509CertificateSSL";
        public MainWindow()
        {
            InitializeComponent();
            hidden_card();
        ...
        /// </summary>
        private  void SendString(string textoutput)
        {
            ...
            try
            {
                ...
                using (var sslStream = new SslStream(client.GetStream(), false, ValidateCertificate))
                {
                    ...
                    byte[] bufferOut = Encoding.UTF8.GetBytes(textoutput);
                    sslStream.Write(bufferOut);
                    Console.WriteLine("\nSent:\n{0}", textoutput);
                    MessageBox.Show("se envio" + textoutput);

                    //*************************************************RRRRRRREEEEEEEECCCCCIIIIIIBBBBBBBBIIIIIIRRRRRRRRRRRRRRRRRRRRRR
                    txtjson.Text = string.Empty;
                    var buffer = new byte[4096];
                    int inputBytes = 0;
                    while (inputBytes == 0)
                    {
                        inputBytes = sslStream.Read(buffer, 0, buffer.Length);
                    }



                    // RECUPERAMOS LOS VALORES QUE NOS LLEGAN
                    string text = Encoding.UTF8.GetString(buffer, 0, inputBytes);
                    string json = "";
